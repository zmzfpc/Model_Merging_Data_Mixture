...
def get_args():
    ...
    p.add_argument("--outfile", type=Path,
                   default="OpenCodeInstruct_filtered.jsonl",
...
def main():
    ...
    if args.split == "all":
        ds_dict: DatasetDict = load_dataset("nvidia/OpenCodeInstruct")      
        dataset = concatenate_datasets(list(ds_dict.values()))
    else:
        dataset = load_dataset("nvidia/OpenCodeInstruct", split=args.split)

    print(f"Rows before filtering: {len(dataset):,}")


    cleaned = dataset.filter(lambda ex: should_drop(ex))             
