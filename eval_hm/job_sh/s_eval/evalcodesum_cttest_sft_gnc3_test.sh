...
models=(
  "/dccstor/unified-trans/model_merging/granite33_2/LLaMA-Factory/saves/gnc3/best/sft_ct_5e6_1"
)
outputs=(
  "gnc3_pred_ct_1.jsonl"
)
...
for i in "${!models[@]}"; do
  model="${models[$i]}"
  save="/dccstor/unified-trans/model_merging/granite33_2/eval_hm/job_out/s_eval/gen_output/${outputs[$i]}_1"
  folder_vis="code_sum/${outputs[$i]}"

  echo "Running eval_codesum.py with model: $model"
  python eval_codesum_1.py \
    --model "$model" \
    --data "$data" \
    --side "$side" \
    --batch_size "$batch" \
