...
# ─────────────────── vLLM generation ──────────────────────────────
def vllm_generate(engine, tok, samples, max_new=1024, temp=0.0, model_name=None):
    print(model_name)
    prompts = [chat_prompt(tok, s["instruction"], s["input"], model_name) for s in samples]
    print(prompts[:1])
    params  = SamplingParams(max_tokens=max_new, temperature=temp, repetition_penalty=1.1)   
...
def evaluate_reg(ref, hyp):
    ...
    rouge = evaluate.load("rouge").compute(predictions=hyp, references=ref)
    meteor= evaluate.load("meteor").compute(predictions=hyp, references=ref)


    return {"BLEU-4": st.mean(bleu), "chrF++": st.mean(chrf),
            "ROUGE-L": rouge["rougeL"], "METEOR": meteor["meteor"]
            }
