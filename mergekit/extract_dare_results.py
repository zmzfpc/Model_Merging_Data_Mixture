#!/usr/bin/env python3
"""
Extract DARE experiment results from output file.
Extracts: Experiment number, Density/Weight parameters, HumanEval/+ scores, MBPP/+ scores, 
...
def extract_dare_results(file_path):
    """Extract DARE experiment results from the output file."""
    ...
    # Split content by experiment sections (DARE uses generic "Experiment" instead of "DARE Experiment")
    experiment_sections = re.split(r'Experiment (\d+)/\d+: \d+', content)
    ...
    for i in range(1, len(experiment_sections), 2):
        ...
        # Extract density and weight parameters
...
def write_to_csv(results, output_file):
    ...
    fieldnames = [
        'input_file', 'method', 'experiment', 'density1', 'weight1', 'density2', 'weight2',
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python extract_dare_results.py <input_file>")
        sys.exit(1)
    
    input_file = sys.argv[1]
    # Extract base filename for output
    base_name = Path(input_file).stem  # gets filename without extension
    output_file = f"csv_output/dare_experiment_results_{base_name}.csv"
    
    if not Path(input_file).exists():
        print(f"Error: Input file '{input_file}' not found")
        sys.exit(1)
    
    print(f"Extracting DARE results from: {input_file}")
    results = extract_dare_results(input_file)
    
    if not results:
        print("No DARE experiment results found")
        sys.exit(1)
    ...
    for result in results:
        print(f"Experiment {result['experiment']}: D1={result['density1']}, W1={result['weight1']}, "
              f"D2={result['density2']}, W2={result['weight2']}, "
