#!/usr/bin/env python3
"""
Extract DELLA experiment results from output file.
Extracts: Experiment number, Density/Weight parameters, Epsilon, HumanEval/+ scores, MBPP/+ scores, 
...
def extract_della_results(file_path):
    """Extract DELLA experiment results from the output file."""
    ...
    experiment_sections = re.split(r'DELLA Experiment (\d+)/9: \d+', content)
    ...
    for i in range(1, len(experiment_sections), 2):
        ...
        # Extract density, weight, and epsilon parameters
        ...
        epsilon_match = re.search(r'Epsilon: ([\d.]+)', section_content)
        ...
        epsilon = epsilon_match.group(1) if epsilon_match else ""
...
def write_to_csv(results, output_file):
    ...
    fieldnames = [
        'input_file', 'method', 'experiment', 'density1', 'weight1', 'density2', 'weight2', 'epsilon',
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python extract_della_results.py <input_file>")
        sys.exit(1)
    
    input_file = sys.argv[1]
    # Extract base filename for output
    base_name = Path(input_file).stem  # gets filename without extension
    output_file = f"csv_output/della_experiment_results_{base_name}.csv"
    
    if not Path(input_file).exists():
        print(f"Error: Input file '{input_file}' not found")
        sys.exit(1)
    
    print(f"Extracting DELLA results from: {input_file}")
    results = extract_della_results(input_file)
    
    if not results:
        print("No DELLA experiment results found")
        sys.exit(1)
    ...
    for result in results:
        print(f"Experiment {result['experiment']}: D1={result['density1']}, W1={result['weight1']}, "
              f"D2={result['density2']}, W2={result['weight2']}, E={result['epsilon']}, "
