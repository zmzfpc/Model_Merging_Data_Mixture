...
# Function to modify YAML parameters in-place using Python (for linear method)
modify_yaml_params() {
    local yaml_file=$1
    local model1_weight=$2
    local model2_weight=$3
...
try:
    ...
    # Modify parameters (linear method only uses weights)
    ...
    print(f"Model 1: weight={$model1_weight}")
    print(f"Model 2: weight={$model2_weight}")
...
modify_yaml_params_sed() {
    ...
    # Use sed to replace parameters (linear method only uses weights)
    ...
    echo "Model 1: weight=$model1_weight"
    echo "Model 2: weight=$model2_weight"
}

# Base YAML file to modify
base_yaml="my_yaml/linear_qwc15_gs.yml"
...
# Format: "model1_weight:model2_weight:description"
...
echo "Starting LINEAR hyperparameter sweep with ${#hyperparams_combinations[@]} combinations..."
...
for i in "${!hyperparams_combinations[@]}"; do
    ...
    IFS=':' read -r model1_weight model2_weight description <<< "${hyperparams_combinations[$i]}"
    
    # Create unique identifier for this combination
    experiment_id="exp${i}_w${model1_weight//./_}_w${model2_weight//./_}"
    output_dir="merged_model/linear_qwc15_gs_${experiment_id}"
    
    echo "=========================================="
    echo "LINEAR Experiment $((i+1))/${#hyperparams_combinations[@]}: ${description}"
    echo "Model 1 - Weight: ${model1_weight}"
    echo "Model 2 - Weight: ${model2_weight}"
    ...
    if command -v python3 &> /dev/null && python3 -c "import yaml" 2>/dev/null; then
        modify_yaml_params "$base_yaml" "$model1_weight" "$model2_weight"
    else
        echo "PyYAML not available, using sed method..."
        modify_yaml_params_sed "$base_yaml" "$model1_weight" "$model2_weight"
    fi
    ...
    if [ $? -eq 0 ]; then
        echo "✓ LINEAR merge successful for experiment $((i+1))"
        ...
        echo "✗ LINEAR merge failed for experiment $((i+1)), skipping evaluation"
    fi

    rm -r "$output_dir"
    
    echo "LINEAR Experiment $((i+1)) complete: ${description}"
    echo ""
done
...
echo "=========================================="
echo "All LINEAR hyperparameter experiments completed!"
echo "Results saved in merged_model/linear_qwc15_gs_exp* directories"
