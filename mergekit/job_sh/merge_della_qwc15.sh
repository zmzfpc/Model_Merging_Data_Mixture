...
# Function to modify YAML parameters in-place using Python (for DELLA method)
modify_yaml_params() {
    local yaml_file=$1
...
try:
    ...
    print(f"Model 1: density={$model1_density}, weight={$model1_weight}, epsilon={$epsilon}")
    print(f"Model 2: density={$model2_density}, weight={$model2_weight}, epsilon={$epsilon}")
...
modify_yaml_params_sed() {
    ...
    sed -i "0,/epsilon:/{s/epsilon: .*/epsilon: $epsilon/}" "$yaml_file"
    ...
    sed -i "0,/epsilon: $epsilon/! {0,/epsilon:/{s/epsilon: .*/epsilon: $epsilon/}}" "$yaml_file"
    
    echo "Updated $yaml_file with sed"
    echo "Model 1: density=$model1_density, weight=$model1_weight, epsilon=$epsilon"
    echo "Model 2: density=$model2_density, weight=$model2_weight, epsilon=$epsilon"
}

# Base YAML file to modify
base_yaml="my_yaml/della_qwc15_gs.yml"
...
# Format: "model1_density:model1_weight:model2_density:model2_weight:epsilon:description"
hyperparams_combinations=(
    "0.5:0.1:0.5:0.9:0.1:1"
    "0.5:0.2:0.5:0.8:0.1:2"
    "0.5:0.3:0.5:0.7:0.1:3"
    "0.5:0.4:0.5:0.6:0.1:4"
    "0.5:0.5:0.5:0.5:0.1:5"
    "0.5:0.6:0.5:0.4:0.1:6"
    "0.5:0.7:0.5:0.3:0.1:7"
    "0.5:0.8:0.5:0.2:0.1:8"
    "0.5:0.9:0.5:0.1:0.1:9"
)

echo "Starting DELLA hyperparameter sweep with ${#hyperparams_combinations[@]} combinations..."
...
for i in "${!hyperparams_combinations[@]}"; do
    ...
    IFS=':' read -r model1_density model1_weight model2_density model2_weight epsilon description <<< "${hyperparams_combinations[$i]}"
    
    # Create unique identifier for this combination
    experiment_id="exp${i}_d${model1_density//./_}_w${model1_weight//./_}_d${model2_density//./_}_w${model2_weight//./_}_e${epsilon//./_}"
    output_dir="merged_model/della_qwc15_gs_${experiment_id}"
    
    echo "=========================================="
    echo "DELLA Experiment $((i+1))/${#hyperparams_combinations[@]}: ${description}"
    ...
    if command -v python3 &> /dev/null && python3 -c "import yaml" 2>/dev/null; then
        modify_yaml_params "$base_yaml" "$model1_density" "$model1_weight" "$model2_density" "$model2_weight" "$epsilon"
    else
        echo "PyYAML not available, using sed method..."
        modify_yaml_params_sed "$base_yaml" "$model1_density" "$model1_weight" "$model2_density" "$model2_weight" "$epsilon"
    fi
    ...
    if [ $? -eq 0 ]; then
        echo "✓ DELLA merge successful for experiment $((i+1))"
        ...
        echo "✗ DELLA merge failed for experiment $((i+1)), skipping evaluation"
    fi

    rm -r "$output_dir"
    
    echo "DELLA Experiment $((i+1)) complete: ${description}"
    echo ""
done
...
echo "=========================================="
echo "All DELLA hyperparameter experiments completed!"
echo "Results saved in merged_model/della_qwc15_gs_exp* directories"
