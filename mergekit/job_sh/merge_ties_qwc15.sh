...
base_yaml="my_yaml/ties_qwc15_gs.yml"
...
echo "Starting TIES hyperparameter sweep with ${#hyperparams_combinations[@]} combinations..."
...
for i in "${!hyperparams_combinations[@]}"; do
    ...
    output_dir="merged_model/ties_qwc15_gs_${experiment_id}"
    
    echo "=========================================="
    echo "TIES Experiment $((i+1))/${#hyperparams_combinations[@]}: ${description}"
    echo "Model 1 - Density: ${model1_density}, Weight: ${model1_weight}"
    echo "Model 2 - Density: ${model2_density}, Weight: ${model2_weight}"
    ...
    if [ $? -eq 0 ]; then
        echo "✓ TIES merge successful for experiment $((i+1))"
        ...
        echo "✗ TIES merge failed for experiment $((i+1)), skipping evaluation"
    fi

    rm -r "$output_dir"
    
    echo "TIES Experiment $((i+1)) complete: ${description}"
    echo ""
done
...
echo "=========================================="
echo "All TIES hyperparameter experiments completed!"
echo "Results saved in merged_model/ties_qwc15_gs_exp* directories"
